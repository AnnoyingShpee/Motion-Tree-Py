...
def test_case(length: int = 10000):
    ...
    lst3 = [random.randint(-10**9, 10**9) for i in range(length)]
...
def solution(x, y):
    ...
    az = sorted(a, key=lambda x: x[2])  # Presorting y-wise
    print("Pre sorted x wise", ax)
    print("Pre sorted y wise", ay)
    print("Pre sorted z wise", az)
    p1, p2, mi = closest_pair(ax, ay, az)  # Recursive D&C function
    return p1, p2, mi


def closest_pair(ax, ay, az):
...
points = test_case(10)
print("Points x", points[0])
print("Points y", points[1])
print(solution(points[0], points[1]))
